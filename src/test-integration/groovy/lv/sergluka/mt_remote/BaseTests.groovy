...
import lv.sergluka.mt_remote.dto.mt4.Mt4ConCommon
import lv.sergluka.mt_remote.dto.mt4.Mt4ConGroup
import lv.sergluka.mt_remote.dto.mt4.Mt4ConGroupMargin
import lv.sergluka.mt_remote.dto.mt4.Mt4ConManager
import lv.sergluka.mt_remote.dto.mt4.Mt4ConManagerSecurity
...
class BaseTests extends BaseSpecification {
    ...
    @Retry // Sometimes MT responds with "General error"
    def "Existing group update"() {
        ...
        sleep(1000) // Wait for Manager API lib synchronises
    ...
    def "Get event at user creation"() {
        ...
        expect:
        StepVerifier.create(client.users().listen().log("LISTEN"))
    ...
    def "Subscribe to EURUSD tick"() {
        ...
        client.market().listen().take(Duration.ofSeconds(5)).blockLast()
